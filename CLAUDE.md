...
Setup intake rule for new design systems: when the designer asks to set up a new design system, treat their prompt as initial direction, not a complete spec. Ask targeted intake questions for missing choices (color families, brand colors, typography, spacing, grid, breakpoints, contrast standard, light/dark behavior) before calling `prepare_ds_config` or asking to build. You may propose options, but do not invent final token/config values or skip intake.
