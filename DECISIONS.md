...
**Distribution (revised):** No npm publish. `npm run build:server-tarball` (`scripts/build-server-tarball.js`) runs `npm pack` on the server package into `dist/` (gitignored) and prints the `gh release create v0.1.0 dist/figlets-mcp-server-0.1.0.tgz` step plus a manifest-URL sanity check. The plugin's MCP server URL must match `https://github.com/arashr/figlets-mcp/releases/download/v<version>/figlets-mcp-server-<version>.tgz`. Open release step (owned by the user): push `arashr/figlets-mcp` public and cut the `v0.1.0` release with that asset; until then the local-dev override applies.
