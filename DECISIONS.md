...
**Always-selectable plugin target:** The `claude-code-plugin` target is always part of the known targets list, so an explicit `--hosts=claude-code-plugin` always returns an actionable plan — when `claude` is missing the status is `manual` with a reason that quotes the exact commands to run after installing Claude Code, and when the marketplace folder is missing the reason names the missing folder. (An earlier iteration of this branch made inclusion conditional on `claude` + marketplace being present, which produced an empty plan and a confusing "rerun with --yes" message for designers who had no Claude Code yet.)
