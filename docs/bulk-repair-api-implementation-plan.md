...
14. **Next (product gaps):** Prune/delete apply, then collection mode creation in `update_ds_tokens`.
15. **Still unsupported by design (product-gap only):** Prune/delete apply; collection mode creation inside `update_ds_tokens`. Primitive categories apply only through `update_ds_primitives`, not `update_ds_tokens` apply.
