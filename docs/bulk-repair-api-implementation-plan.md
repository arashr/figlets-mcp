...
9. **Next (product gaps, not new narrow slices):** Continue from the surface table below; do not reopen the stale-host `{}` symptom unless it reproduces after MCP reconnect on a host that still serves old tool behavior while this script passes.
