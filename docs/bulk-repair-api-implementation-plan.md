...
- Latest local verification: supported-runtime test suite passed **77/77**, and `git diff --check` is clean. The automated E2E-style token flow is `tests/integration/token-gap-planner-flow.test.js`, exercising the foundation-repair handoff plus all seven narrow apply slices end to end. `tests/server/token-update-surface-boundary.test.js` pins the `update_ds_tokens` / `update_ds_primitives` ownership boundary.
...
- **Exact raw elevation finding handoff (2026-07-23):** When `inspect_ds_token_gaps` identifies raw bindings inside existing elevation effect styles, it now emits `effect_style_repairs` in preview/apply inputs. The elevation-only preview consumes that audited payload directly rather than running a second discovery pass, and approved apply forwards the same style ids/names to the bridge. The bridge refreshes only that exact set and fails closed on stale style identities. Category-only fallback previews still use the shared planner, now project `raw-effect-style-bindings` correctly, and suppress healthy generic refreshes when raw findings exist.
...
- **Broad typography/elevation orchestration (2026-05-22):** `update_ds_tokens` apply now accepts broad `typography` and `elevation` categories. The server and bridge expand them into ordered narrow slices (`typography-variables` then `typography-styles`, `elevation-variables` then `elevation-styles`) in one approved call. `inspect_ds_token_gaps` puts broad categories in `repairPlan.applyInput` when both variable and style work exists; otherwise it keeps the existing narrow slice. Broad categories are no longer `unsupported-apply-category` product gaps.
- **Broad typography/elevation live validation (2026-05-22):** Figlets Test (`local_mpcspbgz_7gq8yy0l`) via `scripts/live-validate-broad-orchestration.js` (dev prep `request-remove-text-styles`, broad apply when both slices needed, reinspect). Artifacts: `.local/broad-orchestration-live-validation/`.
- **Token prune + collection modes (2026-05-22):** `update_ds_tokens` dry-run/apply now supports approved `prune.off_config_variables`, `prune.off_config_text_styles`, and `prune.off_config_effect_styles`. Legacy color prune keys redirect to `update_ds_primitives`. **Prune apply requires `prune.config_authoritative=true`** after dry-run review because Figlets compares against the active config, not the full Figma file history. Apply supports `ensure_collection_modes` to add configured breakpoint modes on existing Spacing/Typography collections before responsive writes. `inspect_ds_token_gaps` emits `missing-foundation-modes` and `repairPlan.applyInput.ensure_collection_modes` when synced snapshots show incomplete modes. Live Figlets Test: `scripts/live-validate-ensure-collection-modes.js` (trim modes → inspect → ensure apply → reinspect); `scripts/live-validate-token-prune-and-modes.js` (dry-run candidates, blocked apply without `config_authoritative`, destructive prune opt-in via `FIGLETS_LIVE_APPLY_PRUNE=1`). `ensureReceiverRunning` can restart the receiver with `FIGLETS_DEV_BRIDGE=1` when dev prep endpoints are disabled.
...
- If a specialized Figlets workflow cannot plan or apply a requested bulk repair, the agent must first check whether the exact operation can be represented by `plan_ds_figma_operations`. Only product-specific planning, inference, or designer-decision surfaces should be described as missing Figlets planner/apply scope.
...
| `update_ds_tokens` | Dry-run preview for config-backed non-color completion; approved apply for `radius`, `border-width`, `spacing-semantics`, narrow typography/elevation slices, broad `typography` / `elevation` orchestration, optional `ensure_collection_modes`, and approved off-config token prune | Optional | Apply support covers Spacing collection FLOAT variables `space/radius/*`, `space/border/*`, responsive `space/<semantic>/*` (aliasing primitive spacing when present), Typography collection variables `type/<role>/{size,line-height,weight,tracking,family}`, local text styles derived from `DS.typography.scale`, Elevation collection FLOAT variables `elevation/<key>/{offset-y,radius}`, local effect styles `elevation/0..5`, one-shot broad typography/elevation orchestration, optional breakpoint mode creation on existing Spacing/Typography collections, and approved off-config prune for managed token variables/styles. Color ramp prune stays on `update_ds_primitives`. No primitive typography/shadow apply or arbitrary style mutation. |
...
| `plan_ds_figma_operations` / `apply_ds_figma_operations` | Shared high-level operation layer for exact designer-approved operations on variables, collections, modes, local styles, exact node bindings, metadata, and token lifecycle helpers | Yes | Intended for exact design-system operations and for specialized planners that need basic Figma variable/collection/mode/style/binding/metadata/lifecycle manipulation. It validates against the synced snapshot, marks destructive operations, revalidates approved payloads before apply, and requires sync/reinspect after mutation. It is not a raw arbitrary Figma automation API. |
...
13. **Done:** Broad `typography` / `elevation` one-shot orchestration in `update_ds_tokens` + `inspect_ds_token_gaps` applyInput routing.
14. **Done:** Prune/delete apply and collection mode creation in `update_ds_tokens`. Approved token prune deletes managed off-config variables in Spacing/Typography/Elevation collections and config-derived `type/*` text styles / `elevation/0..5` effect styles only. Color ramp prune stays on `update_ds_primitives`. `inspect_ds_token_gaps` reports `missing-foundation-modes`, routes `repairPlan.applyInput.ensure_collection_modes`, and keeps responsive apply blocked until modes exist or approved ensure runs. Prune apply is blocked unless `prune.config_authoritative=true` after dry-run review. Live-validated on Figlets Test (`scripts/live-validate-token-prune-and-modes.js`, `scripts/live-validate-ensure-collection-modes.js`).
15. **Next (product gaps):** None in the current Phase 3 token-completion slice. Primitive categories apply only through `update_ds_primitives`, not `update_ds_tokens` apply.
...
2. **Done:** Fix stale adapter wording for `update_ds_primitives`.
   - Adapter docs and agent-interface bulk surfaces now list `color`, `spacing`, and `color-semantics`.
   - Regression: `tests/adapter/update-ds-primitives-categories.test.js` and `tests/server/agent-interface-tool.test.js`.

3. **Done:** Add tests that less-capable-agent guidance survives future edits.
   - `tests/server/agent-interface-tool.test.js`
   - `tests/docs/root-agent-entrypoint.test.js`
   - `tests/docs/phase5-workflow-guidance.test.js`
   - `tests/plugins/claude-code-plugin.test.js`
   - `tests/plugins/codex-plugin.test.js`

Implemented notes:

- Added `DESIGNER_FLOW_HARD_RULES.bulkRepairRouting` and `token-gap-completion` workflow in `packages/figlets-mcp-server/src/agent-interface/workflows.js`.
- Synced root `AGENTS.md` / `CLAUDE.md`, adapter docs, and Claude/Codex plugin skills/commands with repair-plan routing and `qa_binding_audit` `fixableNow` guidance.
...
### Phase 3 - Add Config-Backed Token Completion For Non-Color Tokens (Narrow slices done; product gaps remain)
...
Dry-run reports for broad typography/elevation remain useful for designer review. Apply accepts broad `typography` and `elevation` by expanding them into ordered narrow slices in one call. `inspect_ds_token_gaps` includes broad `typography` or `elevation` in `repairPlan.applyInput` when both variable and style work exists; otherwise it keeps the existing narrow slice (`typography-variables`, `typography-styles`, `elevation-variables`, or `elevation-styles`).
...
```js
{
  ...
  dry_run?: boolean,
  ensure_collection_modes?: boolean,
  prune?: {
    off_config_variables?: boolean,
    off_config_text_styles?: boolean,
    off_config_effect_styles?: boolean,
    config_authoritative?: boolean
  }
}
```
...
- Keep `update_ds_primitives` working as the primitive/color-semantic compatibility surface.
- `update_ds_primitives` owns:
...
### Phase 4 - Improve `qa_binding_audit` Bulk Fix Clarity (Done)
...
### Phase 5 - Update Workflow Guidance (Done)
...
- `qa_binding_audit` tells agents what is fixable now versus what needs token creation via `fixability`, `byFixability`, and `repairPlan`.
