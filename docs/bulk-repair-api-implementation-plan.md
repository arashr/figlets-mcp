...
10. **Next (product gaps, not new narrow slices):** Continue from the surface table below for net-new planner/apply gaps.
11. **Still unsupported by design (product-gap only):** Broad `typography`, broad `elevation`, `primitive-typography`, `primitive-shadow`, prune/delete apply, and collection mode creation inside `update_ds_tokens`.
