...
8. **Next (product gaps, not new narrow slices):**
...
9. **Still unsupported by design (product-gap only):** Broad `typography`, broad `elevation`, `primitive-typography`, `primitive-shadow`, prune/delete apply, and collection mode creation inside `update_ds_tokens`.
