...
- **Known dry-run limitation:** On already-complete files, snapshot dry-run for `typography-styles` / `elevation-styles` may report 0 would-refresh entries because the planner only flags missing styles, while live bridge apply can still refresh existing styles in place. Treat that as a future observability improvement, not an apply regression.
...
7. **Next (product gaps, not new narrow slices):**
...
8. **Still unsupported by design (product-gap only):** Broad `typography`, broad `elevation`, `primitive-typography`, `primitive-shadow`, prune/delete apply, and collection mode creation inside `update_ds_tokens`.
