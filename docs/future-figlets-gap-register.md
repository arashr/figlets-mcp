...
|---|---|---|---|---|
| G-001 | No single full health-check tool | Agents can still over-rely on one clean `audit_tokens` result or forget a step | The product rule says health means token audit plus setup/accessibility QA | Add `figlets_health_check` as a read-only orchestrator that runs/summarizes sync, detect, audit, setup gaps, and repair-plan availability. |
...
| G-003 | Host smoke testing is manual | Claude/Codex behavior can drift even when unit tests pass | The product depends on agent compliance, not only server correctness | Add scripted smoke prompts for Claude Code and Codex plugin flows after release packaging. |
