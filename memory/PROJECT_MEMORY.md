...
**Shipped behavior:** `dsConfigToDesignMd` now keeps Google-compatible front matter but expands the canonical `## Colors` body with source color roles, anchor steps, primitive ramp tables with every exported step/hex, contrast algorithm context, and resolved Light/Dark semantic bg/text aliases. The standard `components` front matter also emits compatible `*-dark` variants for semantic pairs when the Dark-mode aliases resolve to standard `colors.*` token references.

**Boundary:** Do not add Figlets-only custom YAML keys to make the file richer. Compatibility remains the Google `@google/design.md` linter. Lossless Figlets-specific data still belongs in the fenced `figlets-extended` block.
...
2. **Rewrote `dsConfigToDesignMd`** in [packages/figlets-core/src/ds-config/design-md-intake.js](packages/figlets-core/src/ds-config/design-md-intake.js):
   ...
   - `components:` block in front matter: each semantic pair → `<slug>: { backgroundColor: {colors.X}, textColor: {colors.Y} }` using Light-mode aliases, plus `<slug>-dark` variants when Dark-mode aliases resolve to standard color tokens.
