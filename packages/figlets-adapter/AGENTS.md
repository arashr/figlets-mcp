...
| `export_design_md` | Syncs the Figma file, refreshes `design-system.config.js` from the latest snapshot, or creates a local snapshot-derived config when none exists, then writes a portable `DESIGN.md` to the opened project's `specs/DESIGN.md` path, with a config-folder fallback or custom `output_path`. Returns `needsDesignerInput` for context Figma cannot encode. | When the designer wants a fresh DESIGN.md handover artifact without re-running the full setup flow. Supports `dry_run` for preview. |
