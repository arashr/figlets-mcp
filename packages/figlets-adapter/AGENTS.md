...
6. If `failCount > 0`: show `semanticPairs.contrastRepairOptions` / `setupApprovalPreview.semanticColor.contrast.repairOptions` as the exact contrast suggestions. Ask the designer to approve or adjust a specific option before changing config. Do not ask vague questions like whether to "keep revising the palette" while structured suggestions exist.
