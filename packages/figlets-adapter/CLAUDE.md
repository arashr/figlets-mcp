...
2. Run any remaining intake: project name, platform, grid base (4px/8px), breakpoints (3-tier/4-tier), semantic color naming grammar (paired context / element-first / intent and emphasis / component-scoped / custom), **contrast standard (APCA default / WCAG 2.2)**, color scale, brand colors (name + hex), typeface, typography preset. Supported typography presets are Material 3 (`material3`, `material`, `standard`, or `material scale`), `fluid`, and `compact`; only `custom` requires an explicit `typography.scale`.
...
6. If `failCount > 0`: show `semanticPairs.contrastRepairOptions` / `setupApprovalPreview.semanticColor.contrast.repairOptions` as the exact contrast suggestions. Ask the designer to approve or adjust a specific option before changing config. Do not ask vague questions like whether to "keep revising the palette" while structured suggestions exist. If the designer approves one or more suggestions, call `apply_ds_config_contrast_repairs` with those option objects, then rerun `prepare_ds_config`.
...
| `prepare_ds_config` returns `failCount > 0` | Semantic pairs fail contrast | Show `semanticPairs.contrastRepairOptions`; after approval call `apply_ds_config_contrast_repairs`, then rerun `prepare_ds_config` |
