...
| `plan_ds_semantic_naming_consolidation` | Dry-run planner for mixed semantic naming conflicts after the designer chooses `surface-based` or `role-based`; lists exact canonical/duplicate variables and safe rename-only payloads | After `inspect_ds_setup_gaps.semanticNamingConflicts` and the designer's canonical naming decision |
