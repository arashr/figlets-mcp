...
For new design system setup, treat my prompt as initial direction, not a complete spec. Ask targeted intake questions for missing choices before calling `prepare_ds_config` or asking to build. You may propose options, but do not invent final token/config values or skip intake.
