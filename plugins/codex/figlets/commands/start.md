...
If `figlets_start` is not available, stop and tell me Figlets is not connected yet. Offer to run `figlets-mcp setup --hosts=codex-plugin --yes` after my approval, or show that command so I can run it myself. Tell me to restart Codex afterwards, then ask again. Do not approximate the flow with raw Figma tools.
