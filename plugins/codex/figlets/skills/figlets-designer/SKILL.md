...
**Stop rule — never edit the repo in Designer Mode:** If Figlets tools cannot complete the request (empty repair payload, apply returns zero changes, missing planner), stop immediately and say that is a Figlets product/tool gap or ask to reconnect/reload Figlets MCP. Do **not** edit figlets-mcp source, the bridge plugin, run `npm test`, or switch to Developer Mode. The designer should not have to tell you “do not edit figlet.”

Use `inspect_ds_setup_gaps.repairPlan.applyInput` for approved repairs. After approval, pass the exact `repairPlan.applyInput` object to `repairPlan.tool` / `apply_ds_setup_repairs`. Never replace `aliases` with counts, summaries, booleans, or prose-derived values. If only a subset is approved, filter entries from `repairPlan.applyInput` while preserving each approved entry's `aliases` object unchanged. If schema validation rejects the payload, stop, rerun `inspect_ds_setup_gaps`, and copy or filter the fresh structured `repairPlan.applyInput` instead of retrying invented arguments. Do not parse Codex/Claude `tool-results`, local snapshots, or transcript files to build repair payloads.

Treat bulk design-system updates as Figlets scope when they can be represented as structured, designer-approved tool payloads. Inspect first. If `repairPlan.applyInput` is non-empty, ask approval and pass it to `repairPlan.tool`; for setup repairs, this means the exact `inspect_ds_setup_gaps.repairPlan.applyInput` object with aliases preserved. If `repairPlan.optionalApplyInput` is non-empty, present it as optional bulk creation requiring separate approval. Use `inspect_ds_setup_gaps.repairPlan` with `apply_ds_setup_repairs` for semantic setup; when `inspect_ds_setup_gaps` reports `semanticNamingConflicts`, ask the designer to choose `surface-based` or `role-based`, then use `plan_ds_semantic_naming_consolidation` and approved `apply_ds_semantic_naming_consolidation`; use `inspect_ds_token_gaps` with `apply_ds_foundation_repairs` and `update_ds_tokens` for config-backed token completion; `update_ds_primitives` for primitive/color-semantic updates; `qa_binding_audit` read-only first, then `qa_binding_audit({ fix: true })` only for `fixableNow` after reading `byFixability`. Do not create tokens from binding-audit findings. If the requested bulk update needs a planner Figlets does not expose yet, say that is a Figlets product/tool gap; do not tell the designer the gaps cannot be fixed as a dead end, and do not write ad hoc scripts to compensate.
