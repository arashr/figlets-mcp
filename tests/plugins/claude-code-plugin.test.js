...
assert.ok(body.includes("interpreted_workflow_id"), "start.md must pass the AI-interpreted canonical workflow id");
assert.ok(/own language/i.test(body), "start.md must make intent routing language-independent");
assert.ok(/do not rely on English keywords/i.test(body), "start.md must not delegate multilingual understanding to text scoring");
